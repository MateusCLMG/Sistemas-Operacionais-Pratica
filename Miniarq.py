...
def interface(dir):
    ...
    while(True):
        ...
        if inp == 1:    #Criar arq
            ...
            listagem = []
            for i,x in enumerate(dir.blocklist):
                if type(dir.blocklist[i]) == str and dir.blocklist[i] != "Livre" and dir.blocklist[i] != dir.blocklist[i-1]:
                    listagem.append((dir.blocklist[i], dir.blocklist.count(x) * blockTam))
            print(listagem)
            print("Diretorios: \n")
            listagemDir = []
            for i,x in enumerate(dir.blocklist):
                if type(dir.blocklist[i]) != str:
                    listagemDir.extend(dir.countDir(dir.blocklist[i]))
